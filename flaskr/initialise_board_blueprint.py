...
def place_settlement():
    update_game_progress.update_game_progress(db.DatabaseConnector, "initial settlement placement")
    settlers = get_settlers.get_settlers(db.DatabaseConnector)
    resources = get_resources.get_resources(db.DatabaseConnector)
...
@bp.route('/place_city')
def place_city():
    update_game_progress.update_game_progress(db.DatabaseConnector, "initial city placement")
    settlers = get_settlers.get_settlers(db.DatabaseConnector)
    resources = get_resources(db.DatabaseConnector)
